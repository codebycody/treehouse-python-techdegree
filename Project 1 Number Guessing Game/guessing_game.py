...
def main():
	...
	def guess_loop(random_number, current_guesses):
		while True:
			try:
				...
				if (guessed_number == random_number):
					# Congradulate the player and tell them the number of tries it took to win
		...
		if (replay_game.lower() == 'y'):
			welcome_setup()
			guess_loop(random.randint(1,10), current_guesses)
